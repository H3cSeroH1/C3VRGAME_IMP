...
{
    //-----------------------------------------------------------------------------
    public class M_SnapTurn : MonoBehaviour
    {
        public float snapAngle = 90.0f;

        public CustomPlayer CustomPlayer = null;
        ...
        private IEnumerator DoRotatePlayer(float angle)
        {
            ...
            Vector3 playerFeetOffset = CustomPlayer.trackingOriginTransform.position - CustomPlayer.feetPositionGuess;
            CustomPlayer.trackingOriginTransform.position -= playerFeetOffset;
            CustomPlayer.transform.Rotate(Vector3.up, angle);
        ...
        private void UpdateOrientation(GameObject fx)
        {
            ...
            this.transform.position = CustomPlayer.hmdTransform.position + (CustomPlayer.hmdTransform.forward * distanceFromFace);
            this.transform.rotation = Quaternion.LookRotation(CustomPlayer.hmdTransform.position - this.transform.position, Vector3.up);
